...
COPY . .
COPY --from=builder-default /build/web/default/dist ./web/default/dist
...
RUN apt-get update \
    && apt-get install -y --no-install-recommends ca-certificates tzdata wget su-exec \
    && rm -rf /var/lib/apt/lists/* \
    && update-ca-certificates \
    && groupadd -r -g 1001 newapi \
    && useradd -r -u 1001 -g newapi -d /data -s /usr/sbin/nologin newapi

...
JSON 参考图兼容 `images`、`image`、`image_urls`、`imageUrls`、`reference_images`、`referenceImages` 和 `image_refs`；进入 Adobe2API 前必须统一去空、去重，再按 9 张上限校验并转发。若客户端误把 JSON 请求标记为不带 boundary 的 `multipart/form-data`，图片入口按 JSON 兼容；真实 multipart 请求缺少 boundary 时返回 400，不尝试猜测分隔符。

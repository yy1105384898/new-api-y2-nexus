...
-- Current public families: nano-banana-pro, nano-banana2 and gpt-image-2.
-- The legacy nano-banana rows stay staged and disabled.
...
BEGIN
    ...
    IF ability_count <> 48 THEN
        RAISE EXCEPTION 'expected 48 Adobe abilities, found %', ability_count;
    END IF;
...
WHERE model_name ~ '^adobe-firefly-(nano-banana-pro|nano-banana2|gpt-image-2)-(1k|2k|4k)$'
  AND deleted_at IS NULL;

UPDATE abilities
SET enabled = TRUE
WHERE channel_id = 75
  AND model ~ '^adobe-firefly-(nano-banana-pro|nano-banana2|gpt-image-2)-(1k|2k|4k)$';

UPDATE models
SET status = 0,
    updated_time = EXTRACT(EPOCH FROM NOW())::BIGINT
WHERE model_name ~ '^adobe-firefly-nano-banana-(1k|2k|4k)$'
  AND deleted_at IS NULL;

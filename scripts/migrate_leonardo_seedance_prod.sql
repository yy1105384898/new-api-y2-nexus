...
FROM (VALUES
    ('cy-sd4-seedance-2.0', 'Leonardo 订阅号 Seedance 2.0。文生/图生/首尾帧，480P/720P/1080P，4–15 秒。', 'video,seedance,subscription'),
...
UPDATE model_ui_param_profiles SET
    option_rules = (
        ...
        FROM (
            ...
            SELECT * FROM jsonb_array_elements('[
                {"param":"reference_videos","disabledWhen":{"modelIncludes":"cy-sd4-seedance"}},
                {"param":"reference_audios","disabledWhen":{"modelIncludes":"cy-sd4-seedance"}},
    ...
    hints = (
        ...
        FROM (
            ...
            UNION ALL
            SELECT * FROM jsonb_array_elements('[
                {"text":"Leonardo 订阅号：文生/图生/首尾帧；暂不支持参考视频/音频（933）。时长 4–15 秒。","when":{"modelIncludes":"cy-sd4-seedance"}}
            ]'::jsonb)
        ) merged(elem)
    ),
    updated_time = EXTRACT(EPOCH FROM NOW())::BIGINT

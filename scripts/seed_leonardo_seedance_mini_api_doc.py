#!/usr/bin/env python3
"""cy-sd4-seedance-2.0-mini：api_doc + ModelPrice + 按次计费（源站 docker 内执行）。"""

import json
import subprocess
import time

MODEL = "cy-sd4-seedance-2.0-mini"
PUBLIC_MODEL = "seedance-2.0-mini"
...
def merge_option_json(key: str, updates: dict[str, object]) -> None:
    raw = psql(f"SELECT value::text FROM options WHERE key='{key}'")
    data = json.loads(raw)
    data.update(updates)
    payload = json.dumps(data, ensure_ascii=False, separators=(",", ":")).replace("'", "''")
    psql_exec(f"UPDATE options SET value='{payload}' WHERE key='{key}'")
    ...
    return {
        ...
        "endpoints": [
            ...
            {"method": "GET", "path": "{{base}}/videos/{task_id}", "description": "查询任务状态与结果。"},
            {
                ...
                "description": "下载成片（亦可直接使用响应中的 video_url）。",
            },
        ],
        "params": [
            {
                "name": "model",
                "description": f"必填，传 public 名 {PUBLIC_MODEL}（与模型广场一致）。",
            },
            ...
            {
                "name": "resolution",
                "description": "清晰度：480p（标准，16:9=864×496）或 720p（HD，16:9=1280×720）。不支持 1080p/4K。",
            },
            ...
            {"name": "reference_audios", "description": "参考音频 HTTPS 直链数组（≤1，≤15 秒）。"},
            {
                "name": "first_image_url",
                "description": "首尾帧：首帧（HTTPS 直链或 data URI；须与 last_image_url 成对，与多模态互斥）。",
            },
            {
                "name": "last_image_url",
                "description": "首尾帧：尾帧（HTTPS 直链或 data URI；须与 first_image_url 成对）。",
            },
        ...
        "examples": [
            ...
            },
            {
                ...
                "request_json": {
                    "_note": (
                        "使用 multipart/form-data，非 JSON body。示例：curl -X POST {{base}}/videos "
                        f"-H 'Authorization: Bearer sk-...' -F model={PUBLIC_MODEL} "
...
def main() -> None:
    ...
    psql_exec(
        ...
        f"updated_time = {ts} WHERE model_name = '{MODEL}' AND deleted_at IS NULL;"
    )
    merge_option_json("ModelPrice", {MODEL: PRICE})
    merge_option_json("billing_setting.billing_mode", {MODEL: "per_request"})
    merge_option_json("billing_setting.request_unit", {MODEL: "generation"})
    print(f"seeded {MODEL}: price={PRICE}, profile={PROFILE}")
